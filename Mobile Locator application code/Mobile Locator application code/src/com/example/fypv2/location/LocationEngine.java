 ...
 * @author A. Egal
 */
class LocationEngine implements LocationFinder {
	//disable passive provider for gps and show dialog for current location
	...
	public LocationEngine(MainContext mainContext) {
	...
	public void findLocation() {
		...
		// Frangart, Eppan, Bozen, Italy:46.480302,11.296005
		//curent location
		hardFix.setLatitude(5.3546718);
		hardFix.setLongitude(100.3010370);
		...
		try {
			...
			// ex2.printStackTrace();
			curLoc = hardFix;
			mainContext.showToast(R.string.connection_GPS_dialog_text);
		}
	}
	...
	private void requestBestLocationUpdates() {
		...
		for (String p : lm.getAllProviders()) {
			if(lm.isProviderEnabled(p)&& !p.equals(LocationManager.PASSIVE_PROVIDER)){
				//force gps
				//mainContext.showToast(p);
	...
	public Location getCurrentLocation() {
		...
		if (curLoc==null) {
			mainContext.showToast(R.string.location_not_found);
			throw new RuntimeException("No GPS Found");

		} else {
			//mainContext.showToast("Current Location: "+curLoc.getLatitude()+" "+curLoc.getLongitude());
		}
	...
	public void setPosition(Location location) {
		...
		mainContext.getMainFrameStatus().refresh();
	...
	public void switchOn() {
		if (!LocationFinderState.Active.equals(state)) {
			lm = (LocationManager) mainContext
		...
		public void run() {
			...
			if(bestLocationProvider != null){
				...
				mainContext.getMainFrameStatus().runOnUiThread(new Runnable() {					
					@Override
					public void run() {
						Toast.makeText(mainContext.getMainFrameStatus(), 
								mainContext.getMainFrameStatus().getResources().getString(
